                       ...
                       int tolerance, int frameFreq)
{
     ...
     */
     ...
     int tempy;
     int count = 0;
     while(!xcoord.isEmpty() && !ycoord.isEmpty() && !p.isEmpty()){
     	tempx = xcoord.remove();
     	tempy = ycoord.remove();
     	curr = p.remove();
     	int distance = pow(curr.red-orig.red, 2) + pow(curr.green-orig.green, 2) + pow(curr.blue-orig.blue, 2);
     	if (distance <= tolerance && !processed[tempx][tempy]){
     		*img(tempx, tempy) = fillColor(tempx,tempy);
     		...
     		if (tempx+1 < width){
     			xcoord.add(tempx+1);
     			ycoord.add(tempy);
     			p.add(*img(tempx+1, tempy));
     		}
     		if (tempy+1 < height){
     			xcoord.add(tempx);
     			ycoord.add(tempy+1);
     			p.add(*img(tempx, tempy+1));
     		}
     		if (tempx-1 >= 0){
     			xcoord.add(tempx-1);
     			ycoord.add(tempy);
